...
<body>
    ...
    <h3 style="text-align: center; margin-top: 10px;">LAPORAN BARANG KEDALUWARSA</h3>
    ...
    <table border="1" style="width: 100%" class="border border-dark">
        <thead>
            <tr>
                ...
                <th class="table-cell text-center">Tanggal Masuk</th>
                <th class="table-cell text-center">Kedaluwarsa</th>s
            </tr>
        </thead>
        <tbody>
            @foreach($dataPersediaan as $bk)
                <tr style="margin: 10px;">
                    <td class="table-cell text-center">{{ $loop->iteration }}</td>
                    <td class="table-cell text-center">{{ $bk->nama_barang }}</td>
                    <td class="table-cell text-center">{{ $bk->tanggal_masuk }}</td>
                    <td class="table-cell text-center">{{ $bk->tanggal_kedaluwarsa }}</td>
                </tr>
                @endforeach
        </tbody>
    </table>
